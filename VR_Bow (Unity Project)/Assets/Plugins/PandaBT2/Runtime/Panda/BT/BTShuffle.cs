...
namespace PandaBT.Runtime
{
	...
	public class BTShuffle : BTProbalisticNode
	{
		protected override Status DoTick ()
		{
			...
			if ( m_selectedChild != null )
			{
				...
				if( status == Status.Succeeded && m_uncompletedChildren.Count > 0 )
					status = Status.Running;
			}

			if (m_selectedChild.status == Status.Succeeded && children.Length > 0 && m_uncompletedChildren.Count == 0)
